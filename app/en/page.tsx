...
export default async function EnglishHome() {
  ...
  return <><Header email={content.company.email} variant="light" locale="en" /><main>
    <section className="hero client-home-hero" style={{ backgroundImage: `url("${content.hero.imageUrl || "/images/jkp-teollisuus-hero-asiakkaan-kuva.jpeg"}")` }}><div className="shell client-home-hero-inner"><div className="client-home-hero-copy">
    ...
    <section className="contact-section" id="contact"><div className="shell contact-grid"><div><p className="eyebrow">Direct contact</p><h2>{content.contactEn.title}</h2><p>{content.contactEn.body}</p>{content.media.contactImageUrl ? <img className="home-contact-image" src={content.media.contactImageUrl} alt="JKP Group contact person" loading="lazy" /> : null}<address className="home-contact-details"><a className="contact-email" href={`mailto:${content.company.email}`}>{content.company.email}</a>{content.company.phone ? <a className="contact-email" href={`tel:${content.company.phone.replace(/\s/g, "")}`}>{content.company.phone}</a> : null}</address></div><ContactForm subject="Enquiry from JKP Group website" locale="en" /></div></section>
